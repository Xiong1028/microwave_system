<?php
/*
 *  Purpose: a class to validate csv files before inserting data into databases
...
class ValidateCSV
{
    ...
    {
        $this->path_general_arr = array_slice($csvArr, 0, 1);
    ...
    {
        for ($i = 0; $i < count($this->path_general_arr[0]) - 1; $i++) {
            switch ($i) {
                case 0:
                    if (!isset($this->path_general_arr[0][$i])) {
                        $this->errors_arr['path01']['path_name'] = "The path_name must exist";
                    ...
                    }else if (strlen(trim($this->path_general_arr[0][$i])) > 100) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    if (strlen($this->path_general_arr[0][$i]) > 65534) {
    ...
    public function validateTable2()
    {
        for ($i = 0; $i < count($this->path_endPoints_arr); $i++) {
            for ($j = 0; $j < count($this->path_endPoints_arr[0]) - 2; $j++) {
                switch ($j) {
                    case 0:
                        if(!isset($this->path_endPoints_arr[$i][$j])){
                            $this->errors_arr['path02'][$i]['dist_from_start'] = "The distance from start of the endPoint must exist";
                        }else if(empty($this->path_endPoints_arr[$i][$j]) && trim($this->path_endPoints_arr[$i][$j]) != '0') {
                            ...
                            $this->errors_arr['path02'][$i]['dist_from_start'] = "The distance from start of the endPoint must be less than 100 or greater than 0";
                        }
                        break;
                    case 1:
                        if(!isset($this->path_endPoints_arr[$i][$j])){
                            $this->errors_arr['path02'][$i]['grd_height'] = "The Ground height must exist";
                        }else if(empty($this->path_endPoints_arr[$i][$j]) && trim($this->path_endPoints_arr[$i][$j]) != '0') {
                    ...
                    case 2:
                        if(!isset($this->path_endPoints_arr[$i][$j])){
                            $this->errors_arr['path02'][$i]['atn_height'] = "The Antenna height must exist";
                        }else if(empty($this->path_endPoints_arr[$i][$j]) && trim($this->path_endPoints_arr[$i][$j]) != '0') {
...
//a method to validate data of table3 path_midPoints
                ...
                switch ($j) {
                    case 0:
                        if (!isset($this->path_midPoints_arr[$i][$j])) {
                            $this->errors_arr['path03'][$i]['dist_from_start'] = 'The distance from start of the midPoint must exist';
                        ...
                        }else if ($this->path_midPoints_arr[$i][$j] > 100 || $this->path_midPoints_arr[$i][$j] < 0) {
                            $this->errors_arr['path03'][$i]['dist_from_start'] = 'The distance from start of the midPoint must be less than 100 or greater than 0';
                        }
                        break;
                    case 1:
                        if(!isset($this->path_midPoints_arr[$i][$j])){
                            $this->errors_arr['path03'][$i]['grd_height'] = "The Ground height of the midPoint must exist";
                        }else if(empty($this->path_midPoints_arr[$i][$j]) && trim($this->path_midPoints_arr[$i][$j])!='0') {
                    ...
                    case 2:
                        if (!isset($this->path_midPoints_arr[$i][$j])){
                            $this->errors_arr['path03'][$i]['grd_height'] = "The terrain type must exist";
                        }else if(empty($this->path_midPoints_arr[$i][$j])&& trim($this->path_midPoints_arr[$i][$j])!='0') {
                    ...
                    case 3:
                        if (!isset($this->path_midPoints_arr[$i][$j])) {
                            $this->errors_arr['path03'][$i]['obstr_height'] = 'The obstruction height must exist';
                        }else if(empty($this->path_midPoints_arr[$i][$j]) && trim($this->path_midPoints_arr[$i][$j]) != '0'){
    ...
    public function get_path_general()
    {
        return $this->path_general_arr;
    }
