...
$selectedPathID = $_SESSION['selectedPath'];
...
if (!is_null($genDataArr['genPathInfoid'])) {
	...
	if (count($err_gen_data)) {
		...
		updateDataAndReponse($db_conn, $qry, $genDataArr['genPathInfoid']);
	}
	...
	if (count($err_mid_data)) {
		...
		updateDataAndReponse($db_conn, $qry, $selectedPathID);
	}
}

disconnect_db($db_conn);

function updateDataAndReponse($db_conn, $qry, $selectedPathID)
{
	$db_conn->query($qry);
	if ($db_conn->errno != 0) {
		echo '{"status":"fail","data":[' . $db_conn->error . " query " . $qry . ']}';
	} else {
		$allData = db_get_all_data($db_conn, $selectedPathID);
		echo '{ "status": "success" ,"data":' . json_encode($allData) . '}';
	}
}
?>